...
import io.swipepay.omniapi.tx.card.preauth.gateway.Omnihubv1117Preauth;

@Service
public class CardPreauthAction extends TxCard {
	
	@Autowired
	private CardPreauthSimulator cardPreauthSimulator;
	...
	@Autowired
	private Omnihubv1117Preauth omnihubv1117Preauth;
	...
	public void saveProcessedTx(PaymentTxCc paymentTxCc) {
		...
		if (paymentTxCc.getTxSuccess()) {
			paymentTxCc.setCapturedAmountRemaining(paymentTxCc.getAmount());
		}
		paymentTxCc.setModified(LocalDateTime.now());
		paymentTxCcRepository.saveAndFlush(paymentTxCc);
	}
	...
	public Boolean routeTx(PaymentTxCc paymentTxCc) {
		...
		if (BooleanUtils.isTrue(omniApiPaymentProperties.getGatewayForceSimulation())) {
			processed = cardPreauthSimulator.simulate(paymentTxCc);
		}
	...
	public Boolean routeTxViaGateway(PaymentTxCc paymentTxCc) {
		...
		for (MerchantProfilePaymentGateway merchantProfilePaymentGateway : merchantProfilePaymentGateways) {
			...
			try {
				...
				case omnihubv1117:
					omnihubv1117Preauth.send(paymentTxCc, merchantProfilePaymentGateway);
					processed = true;
					break;
				default:
					break;
